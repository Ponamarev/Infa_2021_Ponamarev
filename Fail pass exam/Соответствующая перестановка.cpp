#include <iostream>
//Ёта программа должна делать в первом массиве сортировку, а во втором делеать те же самые перестановки.
...
void appstart(int current, List* massive) {
    ...
    */
    ...
    //Ќазначаем элементу указатель на бывший первый элемент массива.
    ptr_new_element->next_ptr = massive->start_ptr;
    //ћен€ем указатель массива на новый первый элемент.
    massive->start_ptr = ptr_new_element;
    massive->lenght ++;
}
    ...
    */
    ...
    if (number <= 0) {
        ...
        if (number == 1){
            ...
            //Ќайдем элемент, идущий до удал€емого.
...
void reverse_list(List* massive) {
    ...
    for (int i = 1; i < massive->lenght; i++)
    {
        ...
        //–азвернем указатель с следующего на предыдущий элемент.
        ptr_element->next_ptr = old_prew_ptr_element;
    }

    //ѕомен€ем указатели массива на конец и на начало местами.
...
void buble_sort(List* massive, bool increase, bool visual) {
    /*
    ѕринимает ссылку на динамический массив massive
    ...
    */
    ...
    while (!(is_massive_sort)) {
        is_massive_sort = true; //≈сли будет перестановка, то помен€етс€ обратно на false.
        for (int i = 1; i < massive->lenght; i++) {
