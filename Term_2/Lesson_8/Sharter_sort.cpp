...
void append(int current, List* massive) {
    ...
    */
    ...
    if (massive->lenght == 0) {
        ...
        ptr_new_element->prew_ptr = massive->end_ptr;
...
void sharter_sort(List* massive, bool increase, bool visual) {
    /*
    Сортирует массив пузырьком, бегающим из угла в угол.
    ВАЖНО. Для этой сортировки был переписан лист в ДВУНАПРАВЛЕННЫЙ.
    Переписаны функции добавление эелемента, отстальные нет.
    */
    int step_number;
    Element* ptr_element = nullptr;
    bool is_massive_sort = false;
    ...
    while (!(is_massive_sort)) {
        ...
        is_massive_sort = true; //Если будет перестановка, то поменяетс обранто на false.
        //Движение пузырька влево.
        for (int i = massive->lenght - 1; i > 1; i--) {
            ptr_element = search_element_with_number(i, massive);
...
int main() {
    ...
    sharter_sort(massive, true, false);
