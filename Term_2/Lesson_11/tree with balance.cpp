...
void balance(Element* tree) {
    /*
    Перераспределяет элементы дерева наиболее эффективным образом.
    */
    if (tree != nullptr && height_of_tree > 1) {
        balance(tree->left_ptr);
        balance(tree->right_ptr);
...
int main() {
    std::vector<int> vect = {25, 50, 35, 70, 15, 10, 22};
    ...
    std::cout << height_of_tree(tree->rode);
